(define-module (gcell backend-webui)
  #:use-module (gcell backend)
  #:use-module (gcell draw)
  #:use-module ((gcell protocol) #:select (<size> size size? size-width
                                            size-height
                                            <mouse> mouse
                                            <resize> resize
                                            <paste> paste))
...
(define-class <webui-backend> (<backend>)
  ...
  ;; image-ids maps a gcell image symbol -> u32 wire id assigned
...
(define-method (backend-record-cycle! (b <webui-backend>) stats)
  ;; STATS is an alist supplied by gcell/engine.scm's event-loop.
...
(define-method (backend-init (b <webui-backend>))
  (let* ((sz   (webui-backend-size-slot b))
    ...
    ;; we want for long-running gcell apps.
    (webui-set-timeout 0)
    ;; Allow more than one tab to connect so reloads and curl probes
    ;; don't kill the server; gcell's input is idempotent so multi-
...
(define (ensure-image-uploaded! b sym)
  "Return the wire id for gcell image symbol SYM, uploading its
...
;; channel (%image-js-fn / gcellImage) -- the client caches by id
;; so we only ship a given image once per session.
(define %image-js-fn "gcellImage")
...
;;;
;;; Input: browser JSON events → gcell protocol msgs.
;;;
...
(define (send-to-engine eng msg)
  ;; Forward declaration trick: gcell engine's `send` proc isn't yet
  ...
  (let ((send-proc (module-ref (resolve-module '(gcell engine)) 'send)))
    (send-proc eng msg)))

(define (forward-log! eng json)
  "Pull \"level\" and \"text\" out of JSON and append a log entry to
ENG via the canonical gcell log channel.  The level falls back to
...
entry on the next frame."
  (let* ((level-str (or (json-field json "level") "info"))
    ...
    (send-to-engine eng ((module-ref (resolve-module '(gcell protocol))
                                     'tick) 0))))

(define (json->protocol-msg b json)
  "Tiny ad-hoc parser for the small JSON shapes the browser emits.
Returns a gcell protocol message or #f.  Avoids pulling in a full
...
user Scheme runs."
  (or (false-if-exception
      ...
      (false-if-exception
       (let ((v (module-variable (current-module) '%gcell-find)))
         (and v (variable-ref v))))))

(define (load-client-script relname)
  "Locate gcell/backend-webui/client/RELNAME and return its
contents as a string.  Looks on disk via %load-path first (dev), then
falls back to the static-binary embed table that gcell-build's
runtime installs (the bytes live at site/3.0/gcell/backend-webui/
client/RELNAME under the canonical embed-path layout)."
  (let* ((rel (string-append "gcell/backend-webui/client/" relname))
         (disk (search-path %load-path rel)))
    (cond
     ((and disk (file-exists? disk))
      (call-with-input-file disk get-string-all))
     (else
      (let* ((find (%resolve-gcell-find))
        ...
        (cond
         ((and bytes (bytevector? bytes))
          (utf8->string bytes))
         (else
          (error "gcell backend-webui: client asset not found"
                 relname))))))))
