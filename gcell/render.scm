(define-module (gcell render)
  #:use-module (gcell view)
  #:use-module (gcell draw)
  #:use-module (gcell faces)
  #:use-module (gcell borders)
  #:use-module (gcell width)
  #:use-module (gcell protocol)
...
RECT-W/H; they only need RECT-COL/ROW to position the caret."
  (cond
   ...
   ((link-node? node)
    (let ((uri (link-node-uri node)))
      (map (lambda (cmd)
             (cond
              ((text-cmd? cmd)
               (make-text (text-col cmd) (text-row cmd) (text-str cmd)
                          (with-hyperlink (text-face cmd) uri)
                          (text-attrs cmd)))
              (else cmd)))
           (view->cmds (link-node-body node) rect mx my))))
   ((semantic-node? node)
    (let ((kind (semantic-node-kind node)))
      (map (lambda (cmd)
             (cond
              ((text-cmd? cmd)
               (make-text (text-col cmd) (text-row cmd) (text-str cmd)
                          (with-semantic (text-face cmd) kind)
                          (text-attrs cmd)))
              (else cmd)))
           (view->cmds (semantic-node-body node) rect mx my))))
   ((hover-node? node)
