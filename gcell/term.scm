(define-module (gcell term)
  #:use-module (gcell term types)
  #:use-module (gcell term ops)
  #:use-module (gcell term sgr)
  #:use-module (gcell term color)
  #:use-module (gcell term write)
  #:use-module (gcell term render)
  #:use-module (gcell term parser)
  #:use-module (gcell term modes)
  #:use-module (gcell term utf8)
  #:use-module (gcell term action)
  #:use-module (gcell term dispatch)
  #:use-module (gcell term base64)
  #:use-module (gcell term selection)
  #:use-module (gcell term snapshot)
  #:re-export
  (<term> make-term term?
   ...
   *modes* mode-def-by-name mode-def-by-key
   ...
   ;; Action / op records dispatched through gcell's `update` generic.
...
;;; Commentary:
;;;
;;; Umbrella for gcell's terminal emulator subsystem.  Importing
;;; (gcell term) gives access to the public surface of the parser,
;;; cell grid, ops, mode state, action/op dispatch, selection model,
;;; snapshot helpers, and the base64 + utf8 codecs the emulator
;;; depends on.  Use (gcell) for the TUI library facade; use
;;; (gcell term) for emulator-as-library consumers.
;;;
;;; Code:
