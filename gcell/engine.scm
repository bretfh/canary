(define-module (gcell engine)
  #:use-module (gcell engine-types)
  #:use-module (gcell widget)
  #:use-module (gcell terminal)
  #:use-module ((gcell protocol) #:select (<size> size size? size-width size-height
  ...
  #:use-module ((gcell key) #:select (<key> key-sym key-mods key-event key=?))
  #:use-module (gcell input)
  #:use-module (gcell backend)
  #:use-module (gcell backend-ansi)
  #:use-module (gcell theme)
  #:use-module (gcell view)
  #:use-module (gcell render)
  #:use-module (gcell keymap)
  #:use-module (gcell keymap-input)
  #:use-module ((gcell draw) #:select (make-clear clickable-cmd?
                                                   clickable-col clickable-row
                                                   clickable-w clickable-h
                                                   clickable-action
                                                   clickable-right-action))
  #:use-module ((gcell layout) #:select (txt vbox width overlay pin))
  #:use-module ((gcell borders) #:select (boxed border-rounded))
  #:use-module (fibers)
  ...
  ;; clash with gcell/protocol's `every` (the timer cmd constructor).
...
are logged, never raised."
  (catch #t
    ...
    (lambda (key . args)
      (format (current-error-port) "gcell render failed: ~a ~a\n" key args))))
...
;;; protocol's press / release events (gcell pushes flag bit 2 in
...
(define %pending-flush-msg    'gcell/chord-pending-flush)
...
(define (write-crash-log eng key args)
  "Dump a crash summary for ENG to $XDG_CACHE_HOME/gcell/last-crash.log
...
predicate isn't exported by (gcell backend-ansi)."
  (is-a? b <ansi-backend>))
