...
extern fn gcell_scm_false() guile.SCM;
extern fn gcell_scm_make_bv(len: usize) guile.SCM;
extern fn gcell_scm_bv_write(bv: guile.SCM, src: [*]const u8, len: usize) void;

fn fibers_epoll_init(_: ?*anyopaque) callconv(.c) void {
    init_fibers_epoll();
}

// Foreign procedure exposed as `%gcell-find` in Guile.  Takes a path
...
// tries to autoload.
fn gcell_find(path_scm: guile.SCM) callconv(.c) guile.SCM {
    const cstr_z = guile.scm_to_utf8_string(path_scm);
    if (cstr_z == null) return gcell_scm_false();
    ...
    if (found) |bytes| {
        const bv = gcell_scm_make_bv(bytes.len);
        gcell_scm_bv_write(bv, bytes.ptr, bytes.len);
        return bv;
    }
    return gcell_scm_false();
}

// Scheme glue: registers %gcell-find from C and overrides
...
fn inner(_: ?*anyopaque, _: c_int, _: [*c][*c]u8) callconv(.c) void {
    ...
    // Expose %gcell-find then install the autoload override.
    _ = guile.scm_c_define_gsubr("%gcell-find", 1, 0, 0, @constCast(@ptrCast(&gcell_find)));
...
pub fn main() !void {
    ...
    if (embed.entries.len == 0) {
        const msg = "gcell-build runtime: built without -Dpayload-dir; refusing to run.\n";
